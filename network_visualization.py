...
def compute_saliency_maps(X, y, model):
    ...
    saliency = gradients.max(dim=1)[0]
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return saliency
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100): 
        ...
        #Update image through the regularized gradient 
        X_fooling.data += learning_rate * (gradient / gradient.norm()) 
...
def update_class_visulization(model, target_y, l2_reg, learning_rate, img):
    ...
    ########################################################################
    scores = model(img) 
    loss = scores.gather(1,torch.LongTensor([target_y]).view(-1,1)).squeeze().sum() 
